...
// Google Play tablet screenshot specs (9:16 portrait, each side within bounds):
//   7-inch:  side range 320-3840px   -> 1080 x 1920
//   10-inch: side range 1080-7680px  -> 1620 x 2880
// Both widths are >=980 CSS px so the app's desktop/tablet sidebar layout renders
// (see CLAUDE.md: "≥980px shows a sidebar + wide content").
const SIZES = [
  { tag: '7in', width: 1080, height: 1920 },
  { tag: '10in', width: 1620, height: 2880 },
]
...
for (const size of SIZES) {
  ...
  for (const s of shots) {
    ...
    await page.screenshot({ path: join(__dirname, `tablet-${size.tag}-${s.name}.png`) })
    console.log(`captured tablet-${size.tag}-${s.name} (${size.width}x${size.height})`)
  }
  await ctx.close()
}
